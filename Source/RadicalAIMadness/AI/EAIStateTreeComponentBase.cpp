#include "EAIStateTreeComponentBase.h"
...
UEAIStateTreeComponentBase::UEAIStateTreeComponentBase(const FObjectInitializer& ObjectInitializer)
...
void UEAIStateTreeComponentBase::InitializeComponent()
{
...
#if WITH_EDITOR
void UEAIStateTreeComponentBase::PostLoad()
{
	Super::PostLoad();
...
void UEAIStateTreeComponentBase::UninitializeComponent()
{
}

bool UEAIStateTreeComponentBase::SetContextRequirements(FStateTreeExecutionContext& Context, bool bLogErrors)
{
	if (!Context.IsValid())
	{
		return false;
...
void UEAIStateTreeComponentBase::BeginPlay()
{
	Super::BeginPlay();
...
void UEAIStateTreeComponentBase::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UEAIStateTreeComponentBase::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
...
void UEAIStateTreeComponentBase::StartLogic()
{
...
void UEAIStateTreeComponentBase::RestartLogic()
{
...
void UEAIStateTreeComponentBase::StopLogic(const FString& Reason)
{
...
void UEAIStateTreeComponentBase::Cleanup()
{
	StopLogic(TEXT("Cleanup"));
}

void UEAIStateTreeComponentBase::PauseLogic(const FString& Reason)
{
	STATETREE_LOG(Log, TEXT("%s: Execution updates: PAUSED (%s)"), ANSI_TO_TCHAR(__FUNCTION__), *Reason);
	bIsPaused = true;
}

EAILogicResuming::Type UEAIStateTreeComponentBase::ResumeLogic(const FString& Reason)
{
...
bool UEAIStateTreeComponentBase::IsRunning() const
{
	return bIsRunning;
}

bool UEAIStateTreeComponentBase::IsPaused() const
{
	return bIsPaused;
}

UGameplayTasksComponent* UEAIStateTreeComponentBase::GetGameplayTasksComponent(const UGameplayTask& Task) const
{
...
AActor* UEAIStateTreeComponentBase::GetGameplayTaskOwner(const UGameplayTask* Task) const
{
	if (Task == nullptr)
	{
		return GetAIOwner();
...
AActor* UEAIStateTreeComponentBase::GetGameplayTaskAvatar(const UGameplayTask* Task) const
{
	if (Task == nullptr)
	{
...
uint8 UEAIStateTreeComponentBase::GetGameplayTaskDefaultPriority() const
{
	return static_cast<uint8>(EAITaskPriority::AutonomousAI);
}

void UEAIStateTreeComponentBase::OnGameplayTaskInitialized(UGameplayTask& Task)
{
	const UAITask* AITask = Cast<const UAITask>(&Task);
...
void UEAIStateTreeComponentBase::SetStartLogicAutomatically(const bool bInStartLogicAutomatically)
{
	bStartLogicAutomatically = bInStartLogicAutomatically;
}

void UEAIStateTreeComponentBase::SendStateTreeEvent(const FStateTreeEvent& Event)
{
	SendStateTreeEvent(Event.Tag, Event.Payload, Event.Origin);
}

void UEAIStateTreeComponentBase::SendStateTreeEvent(const FGameplayTag Tag, const FConstStructView Payload, const FName Origin)
{
	if (!bIsRunning)
	{
...
EStateTreeRunStatus UEAIStateTreeComponentBase::GetStateTreeRunStatus() const
{
...
FString UEAIStateTreeComponentBase::GetDebugInfoString() const
{
